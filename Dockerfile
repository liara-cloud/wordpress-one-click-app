...
FROM wordpress:5.5-php${PHP_VERSION}-apache
...
RUN curl -fsSL 'https://www.sourceguardian.com/loaders/download/loaders.linux-x86_64.tar.gz' -o sourceguardian.tar.gz \
  ...
  && mv /tmp/sourceguardian/ixed.${PHP_VERSION}.lin `php-config --extension-dir` \
  && rm -Rf sourceguardian.tar.gz /tmp/sourceguardian \
  && docker-php-ext-enable ixed.${PHP_VERSION}.lin \
  ...
  && mv /tmp/ioncube/ioncube_loader_lin_${PHP_VERSION}.so `php-config --extension-dir` \
  && rm -Rf ioncube.tar.gz /tmp/ioncube \
  && docker-php-ext-enable ioncube_loader_lin_${PHP_VERSION} \
